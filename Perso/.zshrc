...
if [ -n "$KEYBOARD_ID" ]; then
    alias off="xinput set-prop $KEYBOARD_ID 'Device Enabled' 0 && echo 'Clavier interne ($KEYBOARD_ID) : DÉSACTIVÉ'"
    alias on="xinput set-prop $KEYBOARD_ID 'Device Enabled' 1 && echo 'Clavier interne ($KEYBOARD_ID) : ACTIVÉ'"
else
